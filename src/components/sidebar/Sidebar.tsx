...
import type { PersistedAppState, WorkspaceRecord, ChatSession } from "../../domains/types";
...
export function Sidebar({ state }: SidebarProps) {
  ...
  const filteredWorkspaces = useMemo(() => {
    ...
    return state.workspaces.filter((w) => {
      ...
      return w.name.toLowerCase().includes(normalized) || w.path.toLowerCase().includes(normalized);
    });
  }, [query, state.workspaces]);

  return (
    <aside className="sidebar" style={{ backgroundColor: '#111', color: '#ccc', fontSize: '0.85rem' }}>
      <div className="sidebar__header" style={{ padding: '16px 16px 8px', display: 'flex', alignItems: 'center', borderBottom: 'none' }}>
      ...
      <div style={{ padding: '0 16px', marginBottom: '24px' }}>
        <label style={{ 
          ...
          />
          <div style={{ fontSize: '0.7rem', color: '#666', background: '#222', padding: '2px 4px', borderRadius: '4px' }}>⌘K</div>
        </label>
      </div>

      <div className="sidebar__content" style={{ padding: '0 10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <div style={{ 
          display: 'flex', 
          alignItems: 'center', 
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {filteredWorkspaces.map((workspace) => (
            <ProjectNode key={workspace.id} workspace={workspace} state={state} />
          ))}
        </div>
      </div>

      <div style={{ marginTop: 'auto', padding: '16px' }}>
        <Link to="/settings" style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#888', textDecoration: 'none', fontSize: '0.85rem' }}>
          <Settings size={16} />
          Settings
        </Link>
      </div>
...
function ProjectNode({ workspace, state }: { workspace: WorkspaceRecord, state: PersistedAppState }) {
  const [isExpanded, setIsExpanded] = useState(workspace.id === state.activeWorkspaceId);
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flex: 1 }}>
          ...
          {isExpanded ? <FolderOpen size={14} color="#666" /> : <Folder size={14} color="#666" />}
          <span style={{ fontWeight: workspace.id === state.activeWorkspaceId ? 600 : 400 }}>{workspace.name}</span>
        </div>
        <div className="project-actions" style={{ opacity: 0, transition: 'opacity 0.2s' }}>
           <Plus size={14} onClick={(e) => { e.stopPropagation(); createSession(workspace.id); }} />
        </div>
      </div>

      {isExpanded && (
        <div style={{ display: 'flex', flexDirection: 'column', marginLeft: '12px', borderLeft: '1px solid #333', paddingLeft: '8px', paddingTop: '4px', paddingBottom: '8px' }}>
          {workspace.sessions.length === 0 ? (
            <div style={{ fontSize: '0.75rem', color: '#555', padding: '4px 8px' }}>No threads yet</div>
          ) : (
            workspace.sessions.map((session) => (
              <SessionItem 
                ...
                onSelect={() => selectWorkspaceSession(workspace.id, session.id)}
                onRename={(title) => renameSession(workspace.id, session.id, title)}
...
function SessionItem({ 
  ...
  const menuItems = useMemo<ContextMenuItem[]>(() => [
    ...
    },
    {
      label: "Copy Path",
      icon: <Copy size={14} />,
      onClick: () => navigator.clipboard.writeText(`${workspace.path}/${session.title}`)
    },
    {
  ...
  return (
    <>
      <div
        ...
        onMouseEnter={(e) => { 
          e.currentTarget.style.background = isActive ? 'rgba(255,255,255,0.08)' : '#222';
      ...
      >
        <span className="truncate" style={{ flex: 1, marginRight: '8px' }}>{session.title}</span>
