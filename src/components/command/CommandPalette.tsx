  ...
  const allItems = useMemo(() => {
    ...
    if (mode === "select-workspace") {
      ...
      const filteredItems = items.filter((i) =>
        i.matchText.toLowerCase().includes(deferredQuery),
      );
      return {
    ...
    const actions = [
      {
        ...
        matchText: activeWorkspace
          ? `new thread in ${activeWorkspace.name}`
          : "new thread in",
        run: () => {
          if (activeWorkspace) {
    ...
    const threads = state.workspaces
      .flatMap((workspace) =>
        workspace.sessions
    ...
    const filteredActions = actions.filter((a) =>
      a.matchText.toLowerCase().includes(deferredQuery),
    );
    const filteredThreads = threads.filter((t) =>
      t.matchText.toLowerCase().includes(deferredQuery),
    );
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") {
        ...
        setSelectedIndex((prev) =>
          Math.min(prev + 1, Math.max(0, allItems.flat.length - 1)),
        );
      ...
      return (
        ...
        >
          ...
          >
            {item.title}
            {item.boldSuffix && (
              <span style={{ fontWeight: 600 }}>{item.boldSuffix}</span>
            )}
          </div>
          {item.shortcut && (
            <div
          ...
          {item.showArrow && <ChevronRight size={16} color="var(--text-dim)" />}
        </div>
      );
    }
        ...
        >
          ...
          >
            ...
            <div style={{ color: "#777", fontSize: "0.8rem" }}>
              {item.subtitle
                ? item.subtitle
                : `${item.workspaceName} \u00B7 #${item.branch} ${item.isCurrent ? " \u00B7 Current thread" : ""}`}
            </div>
          </div>
          <div
            style={{
          ...
          {allItems.section1Items.length > 0 && (
            <div style={{ marginBottom: "8px" }}>
              ...
              {allItems.section1Items.map((item) => {
                const globalIndex = allItems.flat.findIndex(
                  (i) => i.id === item.id,
                );
                return renderItem(
                  item,
